...
public class EmoteLoader {
   ...
   private static HashMap<String, Object[]> otherDefaults = new HashMap<>();
      ...
      for(File file : fileArray) {
         if (file.isFile() && file.getName().endsWith(".json")) {
            ...
            for(String key : jsonLines.keySet()) {
               ...
               List<EmoteVariable> variables = new ArrayList<>();

               for(String variableName : variableNames) {
                  variables.add(EmoteVariable.valueOf(variableName.toUpperCase()));
               }

               lines.put(variables, array.toList().toArray(new String[0]));
            }
   ...
   public static void saveOtherDefaults(File dir) throws IOException {
      for(String fileName : otherDefaults.keySet()) {
         ...
         if (!file.exists()) {
            file.getParentFile().mkdirs();
            FileUtils.copyFile((URL)otherDefaults.get(fileName)[0], file);
         }
      }
   }

   public static void registerDefaultEmote(String fileName) throws IOException {
      ClassLoader classLoader = EmotesTF.class.getClassLoader();
